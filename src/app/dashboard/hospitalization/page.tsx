          ...
          <div className="space-y-2">
            ...
            {selectedPatient && (
              <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 text-sm">
                <span className="font-bold text-blue-800">{selectedPatient.name}</span>
...
function TaskItem({ task, onComplete }: { task: any; onComplete: (id: string) => void }) {
  ...
  return (
    <div className="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-100">
      <div className="text-xs font-black text-slate-400 w-12 shrink-0">
        {new Date(task.scheduledTime).toLocaleTimeString("pt-PT", {
          hour: "2-digit",
          minute: "2-digit",
        })}
      </div>
      <div className={`flex-1 font-bold text-sm ${isSkipped ? "line-through text-slate-400" : "text-slate-700"}`}>
        {task.description}
      </div>
      {isDone ? (
        <CheckCircle2 size={18} className="text-green-500 shrink-0" />
      ) : isSkipped ? (
        <span className="text-xs text-slate-400 shrink-0">Ignorado</span>
      ) : (
        <Button
          size="sm"
          className="h-7 rounded-lg text-[10px] font-black uppercase shrink-0"
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-wrap justify-between items-end gap-4">
        ...
        <div className="flex gap-3">
          ...
          <Button
            className="rounded-xl bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-100 font-black gap-2"
