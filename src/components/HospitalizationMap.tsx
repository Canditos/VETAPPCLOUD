    ...
    <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800 overflow-hidden">
      <div className="p-8 border-b border-slate-50 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900">
        <div>
          ...
          <p className="text-xs font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-1">Monitorização em tempo real das boxes</p>
        </div>
        <div className="flex gap-4">
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-emerald-500" />
            <span className="text-[11px] font-black uppercase text-slate-500">Concluído</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-amber-400" />
            <span className="text-[11px] font-black uppercase text-slate-500">Pendente</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-red-500" />
            <span className="text-[11px] font-black uppercase text-slate-500">Atrasado</span>
          </div>
        </div>
      </div>
          ...
          <thead>
            <tr>
              <th className="p-6 text-left sticky left-0 bg-white dark:bg-slate-900 z-20 border-r border-slate-50 dark:border-slate-800 w-64 min-w-[256px]">
                <span className="text-[11px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 dark:text-slate-500">Paciente / Box</span>
              </th>
              {hours.map((hour) => (
                <th 
                  key={hour} 
                ...
                >
                  <span className={`text-xs font-black ${hour === currentHour ? 'text-blue-600 dark:text-blue-400' : 'text-slate-500 dark:text-slate-400 dark:text-slate-500'}`}>
              ...
              <tr key={hosp.id} className="border-t border-slate-50 dark:border-slate-800 group hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors">
                <td className="p-6 sticky left-0 bg-white dark:bg-slate-900 z-10 border-r border-slate-100 dark:border-slate-800 group-hover:bg-slate-50 dark:group-hover:bg-slate-800 transition-colors">
                  <div className="flex items-center gap-3">
                    <Badge className="bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 border-none font-black text-[11px] h-6">
                      {hosp.boxNumber || "BX"}
                    </Badge>
                    <div>
                      <p className="font-black text-slate-900 dark:text-slate-100 text-sm">{hosp.patient.name}</p>
                      <p className="text-[11px] font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase">{hosp.patient.species}</p>
                    </div>
                  </div>
                </td>
                {hours.map((hour) => {
